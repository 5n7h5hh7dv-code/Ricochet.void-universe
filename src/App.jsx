...
const accountMilestones = [
  ...
  "Void Name Chosen",
  "Foundation Started",
  "First Signal Verified",
  "Halfway Through Foundation",
  "Foundation Completed",
  "Reflection Submitted",
  "Waitlist Interest Registered",
  "Entry Access Eligible",
];

const securityEvents = [
  "Local preview session checked",
  "Creator preview gate active",
  "Public launch shield active",
  "Frontend input cleaning active",
  "Security headers configured in Vercel",
  "Anti-indexing protection prepared",
...
export default function App() {
  ...
  const [memberPanel, setMemberPanel] = useState("overview");
  ...
  const accountScore = Math.min(100, Math.round((memberSignedIn ? 20 : 0) + (memberVoidName ? 15 : 0) + (progressPercent * 0.35) + (reflectionSubmitted ? 15 : 0) + (memberWaitlist.length ? 15 : 0)));
  const securityScore = Math.min(100, Math.round(20 + (creatorPreview ? 15 : 0) + (memberSignedIn ? 10 : 0) + (memberEmail ? 10 : 0) + (progressSaved ? 10 : 0) + (checkedItems["Firewall/security headers active"] ? 20 : 0) + (checkedItems["Anti-indexing files active"] ? 15 : 0)));
  const signalRank = foundationComplete ? "Foundation Architect" : currentStep >= 4 ? "Signal Builder" : currentStep >= 1 ? "Path Initiate" : "Unawakened Signal";
    ...
    <main className="pageShell">
      ...
      {!creatorPreview ? (
        ...
        <section className="panel">
          ...
          <div className="hiddenSignal">enhanced member account system added — security grows with the universe.</div>
        </section>
      )}
    </main>
  );
  ...
  return <>
    <div className="card dashboardHero">
      <div className="dashboardMain">
        <div className="cardTitle">Enhanced Member Account System</div>
        <p>This upgraded dashboard prepares the real account experience: identity, signal rank, progress, waitlists, future orders, subscription status, and security state.</p>
        <div className={props.memberSignedIn ? "memberBadge" : "statusRed"}>{props.memberSignedIn ? "Member Preview Active" : "Not Signed In"}</div>
        <div className="memberTabs">
          {["overview", "progress", "collection", "security"].map((panel) => <button key={panel} className={`memberTab ${props.memberPanel === panel ? "active" : ""}`} onClick={() => props.setMemberPanel(panel)}>{panel}</button>)}
        </div>
      </div>
      <div className="dashboardStat"><div className="cardTitle">Signal Rank</div><div className="bigMetric">{props.accountScore}%</div><span>{props.signalRank}</span></div>
    </div>
    ...
    {props.memberPanel === "overview" && <>
      <div className="card grid3"><div className="universeCard greenCard"><strong>Member Identity</strong><span>Name: {props.memberName || "Not entered"}</span><span>Void Name: {props.memberVoidName || "Not chosen"}</span><span>Email: {props.memberEmail || "Not entered"}</span><div className="statusGreen">Preview Identity</div></div><div className="universeCard greenCard"><strong>Account Rank</strong><span>{props.signalRank}</span><span>Signal Score: {props.accountScore}%</span><span>Future: rank tied to verified account progress.</span><div className="statusGreen">Prototype Rank</div></div><div className={props.foundationComplete ? "universeCard greenCard" : "universeCard redCard"}><strong>Entry Access Eligibility</strong><span>{props.foundationComplete ? "Eligible after final review layer." : "Locked until Foundation completion."}</span><span>Future: account-based access unlock.</span><div className={props.foundationComplete ? "statusGreen" : "statusRed"}>{props.foundationComplete ? "Eligible" : "Locked"}</div></div></div>
      <div className="card grid3"><div className="universeCard greenCard"><strong>Subscription Status</strong><span>Current: none connected.</span><span>Future: Entry, Signal, Sub-Creator, Architect Circle, Universe Architect.</span><div className="statusRed">Future Backend</div></div><div className="universeCard greenCard"><strong>Order History</strong><span>No real orders yet.</span><span>Future: pre-orders, rentals, receipts, shipping, collector drops.</span><div className="statusRed">Future Commerce</div></div><div className="universeCard greenCard"><strong>Member Since</strong><span>Preview session only.</span><span>Future: real account creation date.</span><div className="statusGreen">Preview</div></div></div>
    </>}

    {props.memberPanel === "progress" && <>
      <div className="card progressCard"><div className="progressTop"><div>Foundation Account Progress</div><div>{props.progressPercent}%</div></div><div className="progressBar"><div className="progressFill" style={{ width: `${props.progressPercent}%` }}></div></div></div>
      <div className="card grid3"><div className="universeCard greenCard"><strong>Archives Completed</strong><span>{props.currentStep} / {foundationPath.length}</span><span>{props.progressSaved ? "Saved locally on this device." : "Not saved yet."}</span><div className="statusGreen">Local Save</div></div><div className="universeCard greenCard"><strong>Current Signal</strong><span>{props.foundationComplete ? "Foundation Complete" : props.currentArchive.signalTitle}</span><span>{props.foundationComplete ? "Reflection required." : props.currentArchive.difficulty}</span><div className="statusGreen">Active Path</div></div><div className="universeCard greenCard"><strong>Reflection Status</strong><span>{props.reflectionSubmitted ? "Submitted" : "Not submitted"}</span><span>{props.voidName ? `Void Name: ${props.voidName}` : "No Void Name attached yet."}</span><div className={props.reflectionSubmitted ? "statusGreen" : "statusRed"}>{props.reflectionSubmitted ? "Recorded" : "Pending"}</div></div></div>
      <div className="card sectionPad greenPanel"><div className="cardTitle">Milestone Tracker</div>{accountMilestones.map((item) => { const active = (item === "Preview Profile Created" && props.memberSignedIn) || (item === "Void Name Chosen" && props.memberVoidName) || (item === "Foundation Started" && props.currentStep > 0) || (item === "First Signal Verified" && props.currentStep >= 1) || (item === "Halfway Through Foundation" && props.currentStep >= 4) || (item === "Foundation Completed" && props.foundationComplete) || (item === "Reflection Submitted" && props.reflectionSubmitted) || (item === "Waitlist Interest Registered" && props.memberWaitlist.length > 0) || (item === "Entry Access Eligible" && props.foundationComplete); return <div className="securityLog" key={item}><span>{active ? "✅" : "⬜"} {item}</span></div>; })}</div>
    </>}

    {props.memberPanel === "collection" && <>
      ...
      <div className="card grid3"><div className="universeCard greenCard"><strong>Waitlist Status</strong><span>{props.memberWaitlist.length ? props.memberWaitlist.join(", ") : "No waitlist items yet."}</span><div className="statusGreen">Saved Locally</div></div><div className="universeCard redCard"><strong>Collector Record</strong><span>Future collector ID, mint history, artifact ownership, and drop eligibility.</span><div className="statusRed">Future Backend</div></div><div className="universeCard redCard"><strong>Rental / Family Access</strong><span>Future parent read-along rentals and family collection history.</span><div className="statusRed">Future Commerce</div></div></div>
    </>}

    {props.memberPanel === "security" && <>
      <div className="card dashboardHero"><div className="dashboardMain"><div className="cardTitle">Account Security Chamber</div><p>This chamber shows the security architecture we are preparing before real accounts go live.</p><div className={props.securityScore >= 70 ? "memberBadge" : "statusRed"}>{props.securityScore >= 70 ? "Security Foundation Strong" : "Security Still Building"}</div></div><div className="dashboardStat"><div className="cardTitle">Security Score</div><div className="bigMetric">{props.securityScore}%</div><span>Prototype security score only</span></div></div>
      <div className="card grid3"><div className="universeCard greenCard"><strong>Session Status</strong><span>{props.memberSignedIn ? "Preview session active." : "No member preview session."}</span><span>Future: secure server session.</span><div className={props.memberSignedIn ? "statusGreen" : "statusRed"}>{props.memberSignedIn ? "Active" : "Inactive"}</div></div><div className="universeCard redCard"><strong>Multi-Factor Auth</strong><span>Future: email code, authenticator app, recovery codes.</span><div className="statusRed">Future</div></div><div className="universeCard redCard"><strong>Account Recovery</strong><span>Future: verified recovery process and device trust.</span><div className="statusRed">Future</div></div></div>
      <div className="card sectionPad greenPanel"><div className="cardTitle">Security Event Log</div>{securityEvents.map((event) => <div className="securityLog" key={event}><span>{event}</span></div>)}</div>
      <div className="card sectionPad redPanel"><div className="cardTitle restrictedTitle">Backend Security Required</div><p>This frontend chamber is not real authentication. True security still requires backend login, private database rules, server-side validation, protected PDF storage, rate limiting, and deployment protection.</p></div>
    </>}
